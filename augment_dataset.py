...
扩容策略:
  ...
  每张原始图生成 repeats 个曝光版本（repeats = ceil(target / n)），
  V 缩放系数均匀分布在 [0.4, 1.6]：
    target=50000, n≈5637 → repeats=9  → 约 50,724 张
    target=100000,n≈5637 → repeats=18 → 约 101,466 张

扩容范围:
  - train 集: 原始图 × repeats，实际 ≈ n × repeats 张
